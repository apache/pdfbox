...
public class COSArrayList<E> implements List<E>
{
    ...
    public boolean add(E o)
    {
        ...
        //string is a special case because we can't subclass to be COSObjectGetter
        ...
        else
        {
            if(array != null)
            {
                array.add(((COSObjectGetter)o).getCOSObject());
            }
        }
        return actual.add(o);
    }
    ...
    /**
     * This will convert a list of COSObjectGetters to an array list of COSBase objects.
     *
     * @param COSObjectGetterList A list of COSObjectGetter.
     *
     * @return A list of COSBase.
     * @throws IllegalArgumentException if an object type is not supported for conversion to a
     * COSBase object.
     */
    public static COSArray converterToCOSArray(List<?> COSObjectGetterList)
    {
        COSArray array = null;
        if( COSObjectGetterList != null )
        {
            if( COSObjectGetterList instanceof COSArrayList )
            {
                //if it is already a COSArrayList then we don't want to recreate the array, we want to reuse it.
                array = ((COSArrayList<?>)COSObjectGetterList).array;
            }
            else
            {
                array = new COSArray();
                for (Object next : COSObjectGetterList)
                {
                    ...
                    else if( next instanceof COSObjectGetter)
                    {
                        COSObjectGetter object = (COSObjectGetter)next;
    ...
    private List<COSBase> toCOSObjectList( Collection<?> list )
    {
        ...
        list.forEach(next ->
        {
            ...
            {
                COSObjectGetter cos = (COSObjectGetter)next;
    ...
    {
        c.forEach(item -> {
            COSBase itemCOSBase = ((COSObjectGetter)item).getCOSObject();
    ...
    public E set(int index, E element)
    {
        ...
        else
        {
            if( parentDict != null && index == 0 )
            {
                parentDict.setItem( dictKey, ((COSObjectGetter)element).getCOSObject() );
            }
            array.set( index, ((COSObjectGetter)element).getCOSObject() );
        }
    ...
    public void add(int index, E element)
    {
        ...
        {
            array.add( index, ((COSObjectGetter)element).getCOSObject() );
        }
    }
