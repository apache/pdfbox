...
import org.apache.pdfbox.cos.filter.COSInputStream;
// import org.apache.pdfbox.cos.filter.COSOutputStream;
import org.apache.pdfbox.cos.filter.COSOutputStream;
import org.apache.pdfbox.cos.filter.DecodeOptions;
import org.apache.pdfbox.cos.filter.Filter;
import org.apache.pdfbox.cos.filter.FilterFactory;
...
/**
 * This class represents a stream object in a PDF document. This is not an IO
 * stream that can be written to and read from, but a COSStream as defined by
 * the PDF specification which consists of a dictionary followed by zero or more
 * bytes of data that must be decoded before use.
 *
 * The stream dictionary has a required "length" entry that indicates how many bytes
 * are in the stream. This class holds a backing store which is the true IO stream
 * for decoding or encoding the byte data.
 *
...
public class COSStream extends COSDictionary implements Closeable
{
    ...
    {
        return getObjectFromDictionary( COSName.FILTER);
    }
    ...
    {
        try (InputStream input = COSInputStream.create(getFilterList(), this, createRawInputStream() ))
        {
