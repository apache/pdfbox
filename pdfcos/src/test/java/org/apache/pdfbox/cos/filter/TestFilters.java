...
class TestFilters
{
    ...
    void testFilters() // throws IOException
    {
        ...
        // 20 tests; 10 with deterministic seeding, 10 with relatively random.
        for (int iter = 0; iter < COUNT * 2; iter++)
        {
            ...
            else
            {
                ...
                seed = new Random(currentDate.getTime() + iter ).nextLong();
            }
            ...
            try
            {
                ...
                for( Filter filter : FilterFactory.INSTANCE.getAllFilters() )
                {
                    ...
                    try
                    {
                        System.out.println( "Testing " + filter.toString() + "; seed: " + seed % 233 );
                        checkEncodeDecode( filter, original );
                    }
                    catch (IOException e)
                    {
                        System.out.println( "Error checking encoding for filter "
                                                    + filter.toString() + "; continuing" );
                        success = false;
                    }
...
//        if (filter instanceof LZWFilter)
//            original = null;
