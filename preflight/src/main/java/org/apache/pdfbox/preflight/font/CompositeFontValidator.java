...
public class CompositeFontValidator extends AbstractFontValidator {
	...
	public CompositeFontValidator(PreflightContext context, PDFont font)
	...
	protected boolean checkMandatoryFields() {
		...
		String type = this.pFont.getType();
		String subtype = this.pFont.getSubType();

		// just check if they are present because of the Helper has already checked them.
		...
		this.basefont = this.pFont.getBaseFont();
		this.descendantFonts = fDictionary.getItem(COSName.DESCENDANT_FONTS);
		this.encoding = fDictionary.getItem(COSName.ENCODING);

		if ((basefont == null || "".equals(basefont)) || descendantFonts == null || encoding == null) {
			// baseFont syntax isn't checked because of it is a convention not a rule
			this.fontContainer.addError(new ValidationError(ERROR_FONTS_DICTIONARY_INVALID,
	...
	protected boolean checkCIDFont() throws ValidationException {
		...
		COSArray array = COSUtils.getAsArray(descendantFonts, cosDocument);
		if (array == null) {
		...
		this.cidFont = COSUtils.getAsDictionary(array.get(0), cosDocument);
		...
		String type = cidFont.getNameAsString(COSName.TYPE);
		...
		boolean result = checkCIDSystemInfo(sysinfo);
		if (isT0) {
			result = result && checkCIDToGIDMap(cidToGid, false);
			result = result && processCIDFontType0(fontDesc);
		} else {
			result = result && checkCIDToGIDMap(cidToGid, true);
	 ...
	 * @return
	 */
	private boolean checkCIDSystemInfo(COSBase sysinfo) {
	 ...
	 * @return
	 */
	private boolean checkCIDToGIDMap(COSBase ctog,boolean mandatory) {
		if (COSUtils.isString(ctog, cosDocument)) {
			...
			if (!FONT_DICTIONARY_VALUE_CMAP_IDENTITY.equals(ctogStr)) {
				this.fontContainer.addError(new ValidationError(ERROR_FONTS_CIDKEYED_CIDTOGID,"The CIDToGID entry is invalid"));
				return false;
			}
	...
	protected boolean checkCMap(COSBase aEncoding) {
		...
		if (COSUtils.isString(aEncoding, cosDocument)) {
			// ---- if encoding is a string, only 2 values are allowed
			String str = COSUtils.getAsString(aEncoding, cosDocument);
			...
			isIdentityCMap = true;
		} else if (COSUtils.isStream(aEncoding, cosDocument)) {
			...
			return processCMapAsStream(COSUtils.getAsStream(aEncoding, cosDocument));
		} else {
	...
	private boolean compareCIDSystemInfo() {
		if (!isIdentityCMap) {
			COSDictionary cmsi = COSUtils.getAsDictionary(this.cmap.getItem(COSName
					.getPDFName(FONT_DICTIONARY_KEY_CID_SYSINFO)), cosDocument);
			COSDictionary cfsi = COSUtils.getAsDictionary(this.cidFont
					.getItem(COSName.getPDFName(FONT_DICTIONARY_KEY_CID_SYSINFO)), cosDocument);
			String regCM = COSUtils.getAsString(cmsi.getItem(COSName
					.getPDFName(FONT_DICTIONARY_KEY_SYSINFO_REGISTRY)), cosDocument);
			String ordCM = COSUtils.getAsString(cmsi.getItem(COSName
					.getPDFName(FONT_DICTIONARY_KEY_SYSINFO_ORDERING)), cosDocument);
	...
	public void validate() throws ValidationException {
		...
		if (result) {
			this.cmap = COSUtils.getAsStream(encoding, cosDocument);
		}
	...
	protected boolean processCIDFontType0(COSBase fontDesc)	throws ValidationException {
		COSDictionary fontDescDic = COSUtils.getAsDictionary(fontDesc, cosDocument);
		if (fontDescDic == null) {
	 ...
	 * @return
	 */
	boolean checkFontFileElement_CIDFontType0(PDFontDescriptorDictionary pfDescriptor) 
	throws ValidationException {
		// ---- FontFile Validation
