...
public class COSObjectPool {
	private final Map<COSObjectKey, COSBase> keyPool = new HashMap<COSObjectKey, COSBase>();
	private final Map<COSBase, COSObjectKey> objectPool = new HashMap<COSBase, COSObjectKey>();

	private long highestXRefObjectNumber = 0;

	/**
	 * Creates a map of {@link COSBase} instances to {@link COSObjectKey}s, allowing bidirectional lookups.
	 * This constructor can be used for pre - initialized structures to start the assignment of new object numbers
	 * starting from the hereby given offset.
	 *
	 * @param highestXRefObjectNumber The highest known object number.
	 */
	public COSObjectPool(long highestXRefObjectNumber) {
		this.highestXRefObjectNumber = Math.max(highestXRefObjectNumber, this.highestXRefObjectNumber);
	}
	 ...
	 * @return key The {@link COSObjectKey}, that matches the registered {@link COSBase}, or null if such an object
	 * is not registered.
	 */
	...
	public boolean contains(COSBase object) {
		return (object instanceof COSObject && objectPool.containsKey(((COSObject) object).getObject())) ||
