...
public abstract class Encoding implements COSObjectable {
    ...
    public static String getCharacter( String name )
    {
        ...
        if( character == null )
        {
            ...
            // this hexadecimal encoding is used in pdfs generated by TeX/LateX 
            else if (nameStr.startsWith("x")) 
            {
                try {
                    int value = Integer.parseInt(nameStr.substring(1), 16);
                    character = ""+(char)value;
                    NAME_TO_CHARACTER.put(name, character);
                }
                catch(NumberFormatException exception) {
                    log.warn( "Not a number in hexadecimal character name: " + nameStr );
                }
            }
            // this decimal encoding is used in pdfs generated by TeX/LateX 
            else if (nameStr.startsWith("a")) 
            {
                try {
                    int value = Integer.parseInt(nameStr.substring(1));
                    character = ""+(char)value;
                    NAME_TO_CHARACTER.put(name, character);
                }
                catch(NumberFormatException exception) {
                    log.warn( "Not a number in dezimal character name: " + nameStr );
                }
            }
            else 
            {
