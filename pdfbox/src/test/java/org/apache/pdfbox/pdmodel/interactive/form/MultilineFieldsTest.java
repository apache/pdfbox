...
class MultilineFieldsTest
{
    ...
    {
        final PDDocument document = Loader.loadPDF(new File(IN_DIR, "PDFBOX3812-acrobat-multiline-auto.pdf"));
        final PDAcroForm acroForm = document.getDocumentCatalog().getAcroForm();

        // Get and store the field sizes in the original PDF
        final PDTextField fieldMultiline = (PDTextField) acroForm.getField("Multiline");
        final float fontSizeMultiline = getFontSizeFromAppearanceStream(fieldMultiline);

        final PDTextField fieldSingleline = (PDTextField) acroForm.getField("Singleline");
        final float fontSizeSingleline = getFontSizeFromAppearanceStream(fieldSingleline);

        final PDTextField fieldMultilineAutoscale = (PDTextField) acroForm.getField("MultilineAutoscale");
        final float fontSizeMultilineAutoscale = getFontSizeFromAppearanceStream(fieldMultilineAutoscale);

        final PDTextField fieldSinglelineAutoscale = (PDTextField) acroForm.getField("SinglelineAutoscale");
        final float fontSizeSinglelineAutoscale = getFontSizeFromAppearanceStream(fieldSinglelineAutoscale);
    ...
    void testMultilineBreak() throws IOException
    {
        ...
        final PDDocument document = Loader.loadPDF(new File(IN_DIR, TEST_PDF));
        final PDAcroForm acroForm = document.getDocumentCatalog().getAcroForm();

        // Get and store the field sizes in the original PDF
        final PDTextField fieldInput = (PDTextField) acroForm.getField("filled");
        final String fieldValue = fieldInput.getValue();
        final List<String> acrobatLines = getTextLinesFromAppearanceStream(fieldInput);
        fieldInput.setValue(fieldValue);
        final List<String> pdfboxLines = getTextLinesFromAppearanceStream(fieldInput);
    ...
    private float getFontSizeFromAppearanceStream(final PDField field) throws IOException
    {
    	final PDAnnotationWidget widget = field.getWidgets().get(0);
        final PDFStreamParser parser = new PDFStreamParser(widget.getNormalAppearanceStream());
    ...
    private List<String> getTextLinesFromAppearanceStream(final PDField field) throws IOException
    {
    	final PDAnnotationWidget widget = field.getWidgets().get(0);
        final PDFStreamParser parser = new PDFStreamParser(widget.getNormalAppearanceStream());
