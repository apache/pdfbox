...
class EndstreamFilterStreamTest
{
    ...
    void testEndstreamFilterStream() throws IOException
    {
        ...
        final byte[] expectedResult4 = { 1, 2, 3, 4, '\r', '\n', 5, 6, 7, '\r', 8, 9 };
        assertEquals(expectedResult4.length, feos.calculateLength());

        feos = new EndstreamFilterStream();
        final byte[] tab14 = { 1, 2, 3, 4, '\r' };
        final byte[] tab15 = { '\n', 5, 6, 7, '\r' };
        final byte[] tab16 = { 8, 9, '\n' };
        final byte[] tab17 = { '\r' }; // final CR is not to be discarded
        ...
        final byte[] expectedResult5 = { 1, 2, 3, 4, '\r', '\n', 5, 6, 7, '\r', 8, 9, '\n', '\r' };
    ...
    void testPDFBox2079EmbeddedFile() throws IOException
    {
                ...
                new File("src/test/resources/org/apache/pdfbox/pdfparser", "embedded_zip.pdf")))
        {
            ...
            final PDEmbeddedFilesNameTreeNode node = names.getEmbeddedFiles();
            final Map<String, PDComplexFileSpecification> map = node.getNames();
            assertEquals(1, map.size());
            final PDComplexFileSpecification spec = map.get("My first attachment");
            final PDEmbeddedFile file = spec.getEmbeddedFile();
            final InputStream input = file.createInputStream();
            final File d = new File("target/test-output");
            d.mkdirs();
            final File f = new File(d, spec.getFile());
